...
public class ConfigSync
{
    ...
    {
        get => (forceConfigLocking ?? lockedConfig != null &&
    ...
    private void RPC_ConfigSync(long sender, ZPackage package)
    {
        try
        {
            if (isServer && IsLocked)
            {
                bool? exempt =
                    ((SyncedList)AccessTools.DeclaredField(typeof(ZNet), "m_adminList").GetValue(ZNet.instance))
                    ?.Contains(SnatchCurrentlyHandlingRPC.currentRpc?.GetSocket()?.GetHostName());
                if (exempt == false)
                {
                    return;
                }
    ...
    private IEnumerator sendZPackage(List<ZNetPeer> peers, ZPackage package)
    {
        ...
        const int compressMinSize = 5000;
    ...
    private class SendConfigsAfterLogin
    {
        private class BufferingSocket : ISocket
        {
            ...
            public void GetConnectionQuality(out float localQuality, out float remoteQuality, out int ping,
                out float outByteSec, out float inByteSec) => Original.GetConnectionQuality(out localQuality,
                out remoteQuality, out ping, out outByteSec, out inByteSec);
            ...
            public void Send(ZPackage pkg)
            {
                ...
                if ((methodHash == "PeerInfo".GetStableHashCode() || methodHash == "RoutedRPC".GetStableHashCode() ||
                     methodHash == "ZDOData".GetStableHashCode()) && !finished)
                {
    ...
    private static class PreventConfigRereadChangingValues
    {
        ...
        private static bool Prefix(ConfigEntryBase __instance, string value)
        {
            ...
            {
                Debug.LogWarning(
                    $"Config value of setting \"{__instance.Definition}\" could not be parsed and will be ignored. Reason: {e.Message}; Value: {value}");
            }
    ...
    private static ZPackage ConfigsToPackage(IEnumerable<ConfigEntryBase> configs = null,
        IEnumerable<CustomSyncedValueBase> customValues = null, IEnumerable<PackageEntry> packageEntries = null,
        bool partial = true)
    {
        ...
        {
            AddEntryToPackage(package,
                new PackageEntry
                {
                    section = "Internal", key = customValue.Identifier, type = customValue.Type,
                    value = customValue.BoxedValue
                });
        }
        ...
        foreach (ConfigEntryBase config in configList)
        {
            AddEntryToPackage(package,
                new PackageEntry
                {
                    section = config.Definition.Section, key = config.Definition.Key, type = configType(config),
    ...
    private static object ReadValueWithTypeFromZPackage(ZPackage package, Type type)
    {
        ...
        {
            FieldInfo[] fields = cachedValueTypeFields.TryGetValue(type, out FieldInfo[] cachedFields)
                ? cachedFields
                : cachedValueTypeFields[type] =
        ...
        if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Dictionary<,>))
        {
            ...
            FieldInfo keyField = cachedDictionaryKeyFields.TryGetValue(type, out FieldInfo field)
                ? field
                : cachedDictionaryKeyFields[type] =
                    kvType.GetField("key", BindingFlags.NonPublic | BindingFlags.Instance)!;
            FieldInfo valueField = cachedDictionaryValueFields.TryGetValue(type, out FieldInfo field2)
                ? field2
                : cachedDictionaryValueFields[type] =
                    kvType.GetField("value", BindingFlags.NonPublic | BindingFlags.Instance)!;
        ...
        if (type != typeof(List<string>) && type.IsGenericType && type.GetGenericTypeDefinition() == typeof(List<>))
        {
            ...
            MethodInfo adder = cachedListAddMethods.TryGetValue(type, out MethodInfo method)
                ? method
                : cachedListAddMethods[type] = type.GetMethod("Add")!;
            for (int i = 0; i < entriesCount; ++i)
            {
        ...
        cachedClassImpl.SetValue(param, type);
    ...
    private static readonly Dictionary<Type, MethodInfo> cachedListAddMethods = new();
    private static readonly Dictionary<Type, FieldInfo> cachedDictionaryKeyFields = new();
    private static readonly Dictionary<Type, FieldInfo> cachedDictionaryValueFields = new();
    private static readonly Dictionary<Type, FieldInfo[]> cachedValueTypeFields = new();
...
public class VersionCheck
{
    ...
    {
        if (PatchProcessor.GetPatchInfo(AccessTools.DeclaredMethod(typeof(ZNet), "Awake"))?.Postfixes
                .Count(p => p.PatchMethod.DeclaringType == typeof(ConfigSync.RegisterRPCPatch)) > 0)
        {
            return;
        }

        Harmony harmony = new("org.bepinex.helpers.ServerSync");
        foreach (Type type in typeof(ConfigSync).GetNestedTypes(BindingFlags.NonPublic)
                     .Concat(new[] { typeof(VersionCheck) }).Where(t => t.IsClass))
        {
    ...
    static VersionCheck()
    {
        typeof(ThreadingHelper).GetMethod("StartSyncInvoke")!.Invoke(ThreadingHelper.Instance,
            new object[] { (Action)PatchServerSync });
    }
    ...
    private string ErrorClient()
    {
        ...
        return myVersionOk
            ...
            : $"Mod {DisplayName} requires minimum {ReceivedMinimumRequiredVersion}. Installed is version {CurrentVersion}.";
    }

    private string ErrorServer(ZRpc rpc)
    {
        return
            $"Disconnect: The client ({rpc.GetSocket().GetHostName()}) doesn't have the correct {DisplayName} version {MinimumRequiredVersion}";
    }
    ...
    private static void Logout()
    {
        ...
        AccessTools.DeclaredField(typeof(ZNet), "m_connectionStatus")
            .SetValue(null, ZNet.ConnectionStatus.ErrorVersion);
    }
    ...
    private static void CheckVersion(ZRpc rpc, ZPackage pkg, Action<ZRpc, ZPackage> original)
    {
        ...
        foreach (VersionCheck check in versionChecks)
        {
            ...
            Debug.Log(
                $"Received {check.DisplayName} version {currentVersion} and minimum version {minimumRequiredVersion} from the {(ZNet.instance.IsServer() ? "client" : "server")}.");
    ...
    private static void RegisterAndCheckVersion(ZNetPeer peer, ZNet __instance)
    {
        ...
        IDictionary rpcFunctions =
            (IDictionary)typeof(ZRpc).GetField("m_functions", BindingFlags.Instance | BindingFlags.NonPublic)!
                .GetValue(peer.m_rpc);
        if (rpcFunctions.Contains("ServerSync VersionCheck".GetStableHashCode()))
        {
            ...
            Action<ZRpc, ZPackage> action =
                (Action<ZRpc, ZPackage>)function.GetType()
                    .GetField("m_action", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(function);
        ...
        foreach (VersionCheck check in versionChecks)
        {
            ...
            Debug.Log(
                $"Sending {check.DisplayName} version {check.CurrentVersion} and minimum version {check.MinimumRequiredVersion} to the {(__instance.IsServer() ? "client" : "server")}.");
    ...
    private static void ShowConnectionError(FejdStartup __instance)
    {
        if (!__instance.m_connectionFailedPanel.activeSelf ||
            ZNet.GetConnectionStatus() != ZNet.ConnectionStatus.ErrorVersion)
        {
            return;
        }
